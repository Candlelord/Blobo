...
import { Search, Send, Download, Paperclip } from 'lucide-react';
...
const ChatTab = () => {
  ...
  const handleSendMessage = () => {
    if (!message.trim() && !selectedBlob) return;
        ...
        {activeChat ? (
          <>
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  // Open blob selection (integration with BlobsTab or CreateBlobTab needed)
                  setSelectedBlob(blobs[0]); // Example: Select first blob
                  toast.success('Blob attached!');
                }}
